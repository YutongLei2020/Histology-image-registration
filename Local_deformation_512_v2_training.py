...
# path_list = sorted([p for p in os.listdir(path) if p.split('_')[0].isdigit() and 0 <= int(p.split('_')[0]) <= 20])
...
#     fixed_files = sorted([p for p in os.listdir(path+f+'/patches_512/fixed/') if p.split('_')[0] == 'patch'])
#     moving_files = sorted([p for p in os.listdir(path+f+'/patches_512/moving/') if p.split('_')[0] == 'patch'])
#     for i in range(len(fixed_files)):
#         fixed_path = f"{path}{f}/patches_512/moving/{fixed_files[i]}"
#         moving_path = f"{path}{f}/patches_512/fixed/{moving_files[i]}"
#         # print(fixed_path)
#         # print(moving_path)
#         # print('>>',f,': ',i)
...
#         moving_patch = cv2.imread(moving_path).astype(float)
#         fixed_patch = cv2.imread(fixed_path).astype(float)

#         moving_patch_bw = moving_patch.mean(axis=2)
#         fixed_patch_bw = fixed_patch.mean(axis=2)

#         moving_patch_bw = moving_patch_bw / 255.0 if moving_patch_bw.max() > 1 else moving_patch_bw
#         fixed_patch_bw = fixed_patch_bw / 255.0 if fixed_patch_bw.max() > 1 else fixed_patch_bw

#         fixed_tensor = torch.from_numpy(fixed_patch_bw).float().unsqueeze(0)
#         moving_tensor = torch.from_numpy(moving_patch_bw).float().unsqueeze(0)
#         print(ncc_loss(fixed_tensor.unsqueeze(0), moving_tensor.unsqueeze(0)))
...
# dataset = TensorPairDataset(fixed_list, moving_list)
...
for s in stains:
    ...
    # path_list = sorted([p for p in os.listdir(path) if p.split('_')[0].isdigit() and 1 <= int(p.split('_')[0]) <= 20])
    # path_list = sorted([p for p in os.listdir(path) if p.split('_')[-1].isdigit() and 0 <= int(p.split('_')[-1]) <= 120])
    path_list = sorted([p for p in os.listdir(path) if p.split('_')[-1].isdigit() and 10 <= int(p.split('_')[-1]) <= 18])
    for f in path_list:
        print(f"Processing {f}...")
        
        if not os.path.exists(path+f+'/patches_512_registered3/fixed/') or not os.path.exists(path+f+'/patches_512_registered3/moving/'):
            print(f"Missing files for {path+f+'/patches_512_registered3/fixed/'}, skipping.")
            continue
        fixed_files = sorted([p for p in os.listdir(path+f+'/patches_512_registered3/fixed/') if p.split('_')[0] == 'patch'])
        moving_files = sorted([p for p in os.listdir(path+f+'/patches_512_registered3/moving/') if p.split('_')[0] == 'patch'])
    
        for i in range(len(fixed_files)):
            fixed_path = f"{path}{f}/patches_512_registered3/fixed/{fixed_files[i]}"
            moving_path = f"{path}{f}/patches_512_registered3/moving/{moving_files[i]}"
            ...
            # Compute NCC
            with torch.no_grad():
                score = ncc_loss(fixed_tensor, moving_tensor).item() # flip sign back to get actual NCC
            ...
            if torch.isnan(fixed_tensor.squeeze(0)).any():
                print("fixed contains NaNs!")
                continue
            if torch.isnan(moving_tensor.squeeze(0)).any():
...
train_loader = balanced_batch_loader(bucket_dataloaders["low"].dataset, 
...
#     path = f'/extra/zhanglab0/INDV/leiy28/image_registration/augmented_acrobat_{s}_both/'
#     if s == 'HE':
#         path_list = sorted([p for p in os.listdir(path) if p.split('_')[0].isdigit() and 1 <= int(p.split('_')[0]) <= 300])
#     else:
...
# checkpoint_path = "/extra/zhanglab0/INDV/leiy28/image_registration/global_deform/test1/checkpoint_real_local_512_v2_more.pth"
checkpoint_path = "/extra/zhanglab0/INDV/leiy28/image_registration/global_deform/test1/checkpoint_real_local_512_v3.pth"
...
for epoch in range(num_epochs):
    ...
    count = 0
    # for i, (fixed, moving) in enumerate(train_loader):
    ...
    for i in range(num_batches):
        ...
        # del fixed, moving, fixed_center, moving_center, pred_flow, loss
        # torch.cuda.empty_cache()  # for GPU memory
        # gc.collect()              # for CPU memory

    # avg_epoch_loss = epoch_loss / len(num_batches)
    avg_epoch_loss = epoch_loss / num_batches
    loss_history.append(avg_epoch_loss)
    # print(f"Epoch {epoch+1} - Avg Loss: {epoch_loss / len(num_batches):.6f}")
    print(f"Epoch {epoch+1} - Avg Loss: {epoch_loss / num_batches:.6f}")

save_path = "/extra/zhanglab0/INDV/leiy28/image_registration/global_deform/test1/checkpoint_real_local_512_v3.pth"
torch.save({
    'epoch': epoch,
